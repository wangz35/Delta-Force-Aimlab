...
void AAimTrainerHUD::DrawHUD()
{
    ...
    const bool bSlowWalking = TrainingPawn && TrainingPawn->IsSlowWalking();
    ...
    const FLinearColor AccentGreen(0.18f, 1.0f, 0.34f, 1.0f);
    const FLinearColor CrosshairRed(1.0f, 0.02f, 0.02f, 1.0f);
    ...
    Canvas->K2_DrawBox(FVector2D(30.0f, 26.0f), FVector2D(4.0f, 90.0f), 1.0f, AccentGreen);
    ...
    Canvas->K2_DrawBox(FVector2D(MidX - 2.0f, CrosshairY - 2.0f), FVector2D(4.0f, 4.0f), 1.0f, CrosshairRed);
    ...
    DrawLabel(FString::Printf(TEXT("WASD MOVE  /  CAPS %s  /  SHIFT SPRINT  /  SPACE JUMP  /  SLIDE + SPACE SLIDE-JUMP  /  [ ] SENS %.2fx  /  RMB ZOOM  /  R RESTART"), bSlowWalking ? TEXT("SLOW") : TEXT("NORMAL"), Sensitivity), FVector2D(30.0f, Height - 28.0f), PaleOrange, 0.68f);
